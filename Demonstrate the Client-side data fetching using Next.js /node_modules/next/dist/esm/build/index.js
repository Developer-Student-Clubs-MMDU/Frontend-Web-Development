...
export default async function build(dir, conf = null, reactProductionProfiling = false, debugOutput = false, runLint = true) {
    try {
        ...
        const buildResult = await nextBuildSpan.traceAsyncFn(async ()=>{
            ...
            telemetry.record(eventCliSession(dir, config, {
                webpackVersion: 5,
                cliCommand: "build",
                isSrcDir: !!pagesDir && path.relative(dir, pagesDir).startsWith("src") || !!appDir && path.relative(dir, appDir).startsWith("src"),
            ...
            const rootPaths = pagesDir ? (await flatReaddir(join(pagesDir, ".."), middlewareDetectionRegExp)).map((absoluteFile)=>absoluteFile.replace(dir, "")) : [];
            ...
            const pageKeys = {
                ...
                app: mappedAppPages ? Object.keys(mappedAppPages).map((key)=>normalizeAppPath(key) || "/") : undefined
            };
            if (pageKeys.app) {
                ...
                await Promise.all(Object.entries(pageKeys).reduce((acc, [key, files])=>{
                    ...
                    return checkPageSpan.traceAsyncFn(async ()=>{
                        ...
                        const staticInfo = pagePath ? await getPageStaticInfo({
                            pageFilePath: join((pageType === "pages" ? pagesDir : appDir) || "", pagePath),
                            nextConfig: config
                        }) : undefined;
                        ...
                        if (!isReservedPage(page)) {
                            try {
                                ...
                                if (pageRuntime === SERVER_RUNTIME.edge) {
                                    const manifest = require(join(distDir, SERVER_DIRECTORY, MIDDLEWARE_MANIFEST));
            ...
            if (config.output === "standalone") {
                await nextBuildSpan.traceChild("copy-traced-files").traceAsyncFn(async ()=>{
                    await copyTracedFiles(dir, distDir, pageKeys.pages, outputFileTracingRoot, requiredServerFiles.config, middlewareManifest);
                });
            }
            ...
            if (Boolean(config.experimental.nextScriptWorkers)) {
                await nextBuildSpan.traceChild("verify-partytown-setup").traceAsyncFn(async ()=>{
                    await verifyPartytownSetup(dir, join(distDir, CLIENT_STATIC_FILES_PATH));
                });
            }
