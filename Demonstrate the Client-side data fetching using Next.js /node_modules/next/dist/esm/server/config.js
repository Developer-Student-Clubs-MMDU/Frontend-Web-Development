...
export function setHttpClientAndAgentOptions(options) {
    var ref;
    if (semverGte(process.version, "16.8.0")) {
        var ref1;
        if (((ref1 = options.experimental) == null ? void 0 : ref1.enableUndici) && semverGte(process.version, "18.0.0")) {
    ...
    global.__NEXT_HTTP_AGENT = new HttpAgent(options.httpAgentOptions);
    global.__NEXT_HTTPS_AGENT = new HttpsAgent(options.httpAgentOptions);
}
async function setFontLoaderDefaults(config, dir) {
    ...
    const hasNextFontDependency = (await getDependencies({
        cwd: dir
...
function assignDefaults(dir, userConfig) {
    ...
    if (((ref3 = result.experimental) == null ? void 0 : ref3.outputFileTracingRoot) && !isAbsolute(result.experimental.outputFileTracingRoot)) {
