...
function createTSPlugin(modules) {
    ...
    function create(info) {
        const appDir = _path.default.join(info.project.getCurrentDirectory(), "app");
        const isAppEntryFile = (filePath)=>{
            return filePath.startsWith(appDir) && /(page|layout)\.(mjs|js|jsx|ts|tsx)$/.test(_path.default.basename(filePath));
        };
                                ...
                                if (ts.isIdentifier(name)) {
                                    if (!ALLOWED_EXPORTS.includes(name.text) && !API_DOCS[name.text]) {
                                        ...
                                        if (value) {
                                            if (value.kind === ts.SyntaxKind.StringLiteral) {
                                            ...
                                            } else if (value.kind === ts.SyntaxKind.NumericLiteral || value.kind === ts.SyntaxKind.PrefixUnaryExpression && value.operator === ts.SyntaxKind.MinusToken && value.operand.kind === ts.SyntaxKind.NumericLiteral || value.kind === ts.SyntaxKind.Identifier && value.getText() === "Infinity") {
                                                var _text, ref3;
                                                ...
                                                prior.push({
                                                    ...
                                                    category: ts.DiagnosticCategory.Error,
                                                    code: 71003,
                                                    messageText: `"${value.getText()}" is not a valid value for the "${name.text}" option.`,
                                                    start: value.getStart(),
                                                    length: value.getEnd() - value.getStart()
                                                });
                                            }
