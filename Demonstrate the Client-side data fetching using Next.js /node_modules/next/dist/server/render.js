...
async function renderToString(element) {
    if (!_utils1.shouldUseReactRoot) return ReactDOMServer.renderToString(element);
    const renderStream = await ReactDOMServer.renderToReadableStream(element);
    await renderStream.allReady;
    return (0, _nodeWebStreamsHelper).streamToString(renderStream);
}
async function renderToStaticMarkup(element) {
    if (!_utils1.shouldUseReactRoot) return ReactDOMServer.renderToStaticMarkup(element);
